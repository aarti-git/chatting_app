<template>
    <div>
        <aside  class="main">
            <div class="main__wrapper">
                <a href="/" class="main__logo">
                    <svg class="main__large-svg-icons" aria-hidden="true" focusable="false" data-prefix="fal" data-icon="comment-lines" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                        <path fill="currentColor" d="M280 272H136c-4.4 0-8 3.6-8 8v16c0 4.4 3.6 8 8 8h144c4.4 0 8-3.6 8-8v-16c0-4.4-3.6-8-8-8zm96-96H136c-4.4 0-8 3.6-8 8v16c0 4.4 3.6 8 8 8h240c4.4 0 8-3.6 8-8v-16c0-4.4-3.6-8-8-8zM256 32C114.6 32 0 125.1 0 240c0 47.6 19.9 91.2 52.9 126.3C38 405.7 7 439.1 6.5 439.5c-6.6 7-8.4 17.2-4.6 26S14.4 480 24 480c61.5 0 110-25.7 139.1-46.3C192 442.8 223.2 448 256 448c141.4 0 256-93.1 256-208S397.4 32 256 32zm0 384c-28.3 0-56.3-4.3-83.2-12.8l-15.2-4.8-13 9.2c-23 16.3-58.5 35.3-102.6 39.6 12-15.1 29.8-40.4 40.8-69.6l7.1-18.7-13.7-14.6C47.3 313.7 32 277.6 32 240c0-97 100.5-176 224-176s224 79 224 176-100.5 176-224 176z"></path>
                    </svg>
                </a>
                <div class="main__setting-options">
                    <ul class="main__list">
                        <li class="main__list-item" @click="activeMenu(1)" :class="{active : active_el == 1}">
                            <svg class="main__svg-icons"  aria-hidden="true" focusable="false" data-prefix="fal" data-icon="comment" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                <path fill="currentColor" d="M256 64c123.5 0 224 79 224 176S379.5 416 256 416c-28.3 0-56.3-4.3-83.2-12.8l-15.2-4.8-13 9.2c-23 16.3-58.5 35.3-102.6 39.6 12-15.1 29.8-40.4 40.8-69.6l7.1-18.7-13.7-14.6C47.3 313.7 32 277.6 32 240c0-97 100.5-176 224-176m0-32C114.6 32 0 125.1 0 240c0 47.6 19.9 91.2 52.9 126.3C38 405.7 7 439.1 6.5 439.5c-6.6 7-8.4 17.2-4.6 26 3.8 8.8 12.4 14.5 22 14.5 61.5 0 110-25.7 139.1-46.3 29 9.1 60.2 14.3 93 14.3 141.4 0 256-93.1 256-208S397.4 32 256 32z"></path>
                            </svg>
                        </li>
                        <li class="main__list-item" @click="activeMenu(2)" :class="{active : active_el == 2}">
                            <svg class="main__svg-icons"  aria-hidden="true" focusable="false" data-prefix="fal" data-icon="user" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                <path fill="currentColor" d="M313.6 288c-28.7 0-42.5 16-89.6 16-47.1 0-60.8-16-89.6-16C60.2 288 0 348.2 0 422.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-41.6c0-74.2-60.2-134.4-134.4-134.4zM416 464c0 8.8-7.2 16-16 16H48c-8.8 0-16-7.2-16-16v-41.6C32 365.9 77.9 320 134.4 320c19.6 0 39.1 16 89.6 16 50.4 0 70-16 89.6-16 56.5 0 102.4 45.9 102.4 102.4V464zM224 256c70.7 0 128-57.3 128-128S294.7 0 224 0 96 57.3 96 128s57.3 128 128 128zm0-224c52.9 0 96 43.1 96 96s-43.1 96-96 96-96-43.1-96-96 43.1-96 96-96z"></path>
                            </svg>
                        </li>
                        <li class="main__list-item" @click="activeMenu(3)" :class="{active : active_el == 3}">
                            <svg class="main__svg-icons" aria-hidden="true" focusable="false" data-prefix="fal" data-icon="star" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
                                <path fill="currentColor" d="M528.1 171.5L382 150.2 316.7 17.8c-11.7-23.6-45.6-23.9-57.4 0L194 150.2 47.9 171.5c-26.2 3.8-36.7 36.1-17.7 54.6l105.7 103-25 145.5c-4.5 26.3 23.2 46 46.4 33.7L288 439.6l130.7 68.7c23.2 12.2 50.9-7.4 46.4-33.7l-25-145.5 105.7-103c19-18.5 8.5-50.8-17.7-54.6zM405.8 317.9l27.8 162L288 403.5 142.5 480l27.8-162L52.5 203.1l162.7-23.6L288 32l72.8 147.5 162.7 23.6-117.7 114.8z"></path>
                            </svg>
                        </li>
                        <li class="main__list-item" @click="activeMenu(4)" :class="{active : active_el == 4}">
                            <svg class="main__svg-icons" aria-hidden="true" focusable="false" data-prefix="fal" data-icon="archive" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                <path fill="currentColor" d="M464 32H48C21.5 32 0 53.5 0 80v64c0 8.8 7.2 16 16 16h16v272c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48V160h16c8.8 0 16-7.2 16-16V80c0-26.5-21.5-48-48-48zm-16 400c0 8.8-7.2 16-16 16H80c-8.8 0-16-7.2-16-16V160h384v272zm32-304H32V80c0-8.8 7.2-16 16-16h416c8.8 0 16 7.2 16 16v48zM204 256h104c6.6 0 12-5.4 12-12v-8c0-6.6-5.4-12-12-12H204c-6.6 0-12 5.4-12 12v8c0 6.6 5.4 12 12 12z" class=""></path>
                            </svg>
                        </li>
                    </ul>
                    <div class="main__profile-parent">
                        <div class="main__list-item">
                            <svg class="main__svg-icons" aria-hidden="true" focusable="false" data-prefix="fal" data-icon="moon" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                <path fill="currentColor" d="M448.964 365.617C348.188 384.809 255.14 307.765 255.14 205.419c0-58.893 31.561-112.832 82.574-141.862 25.83-14.7 19.333-53.859-10.015-59.28A258.114 258.114 0 0 0 280.947 0c-141.334 0-256 114.546-256 256 0 141.334 114.547 256 256 256 78.931 0 151.079-35.924 198.85-94.783 18.846-23.22-1.706-57.149-30.833-51.6zM280.947 480c-123.712 0-224-100.288-224-224s100.288-224 224-224c13.984 0 27.665 1.294 40.94 3.745-58.972 33.56-98.747 96.969-98.747 169.674 0 122.606 111.613 214.523 231.81 191.632C413.881 447.653 351.196 480 280.947 480z"></path>
                            </svg>
                        </div>
                        <img class="main__profile-img" src="../assets/image/aarti.jpg" @click="toggleSetting">
                        <div v-if="showSetting" class="main__profile-Setting-wrapper">
                            <div class="main__setting-list">
                                <div 
                                    v-for="option in settingOptions"
                                    :key="option.title" @click="openSettingsOverlay(option.click)"
                                >
                                    <router-link :to="option.link" class="main__setting-item-links">
                                        <p class="main__setting-item">
                                            {{option.title}}
                                        </p>
                                    </router-link> 
                                </div>
                            </div>
                        </div>
                    </div>   
                </div>
            </div>
        </aside>
        <div v-if="openSetting" class="user-list__addgroup-overlay">
            <Settings @close="openSetting = false"/>
        </div> 
         <div v-if="openEditProfile" class="user-list__addgroup-overlay">
            <EditProfile @close="openEditProfile = false"/>
        </div> 
    </div>
</template>


<!-- script -->

<script>
import Settings from './Setting.vue';
import EditProfile from './EditProfile.vue';
import { mapMutations } from 'vuex'

export default {
    name: "Main",
    components:{
        Settings,
        EditProfile,
    },
    data(){
       return{
            active_el:1,
            openSetting:false,
            openEditProfile:false,
            settingOptions:[
               {
                   title:"Edit profile",
                   click:1,
                    link:"/"
               },
               {
                   title:"profile",
                   click:2,
                    link:"/"
               },
               {
                   title:"Settings",
                   click:3,
                    link:"/"
               },
               {
                   title:"Log out",
                   click:4,
                   link:"/login"
               },
           ],
           showSetting:false,
       }
   },
   methods: {
       ...mapMutations(['toggleProfile']),
		toggleSetting() {
			this.showSetting = !this.showSetting
		},
        openSettingsOverlay(no){
            console.log(no)
            if(no == 1){
                this.openEditProfile = true;
            }else if(no == 2){
                this.toggleProfile(true);
            }else if(no == 3){
                this.openSetting = true;
            }
            if(no !== 4){
                this.showSetting = false;
                var Home = document.querySelector(".home");
                var overlay = document.querySelector(".overlay");
                Home.style.transform="translateX(0px)";
                if(overlay){
                    overlay.remove();
                }
            }
            
        },
        activeMenu(el){
            this.active_el = el;
        }
   }
}

</script>


<style lang="scss" scoped>
    @import '../design';
    .main{
        background-color: var(--color1);
        z-index: 1;
        position: fixed;
        top: 0;
        left: -100px;
        z-index: 1;
        @include above('tablet-large'){
            position:unset;
            left: 0;
        }
    }
    .main__wrapper{
        width: 100px;
        height: 100vh;
        border-right: 1px solid var(--border-color);
        text-align: center;
        display: flex;
        flex-direction: column;
    }
    .main__logo{
        padding: 20px 0;
    }
    .main__setting-options{
        display: flex;
        flex-direction: column;
        justify-content: space-between;
        flex-grow: 1;
    }
    .main__list{
       list-style: none;
       padding: 0; 
    }
    .main__list-item{
        padding: 15px 0;
        margin: 10px 18px;
    }
    .main__svg-icons:hover{
        color: var(--color3);
    }
    .main__svg-icons{
        width: 18px;
        height: 18px;
        color:var(--color2);
        cursor: pointer;
    }
    .main__large-svg-icons{
        width: 30px;
        height: 30px;
        color:var(--color3);
    }
    .main__profile-parent{
        margin-bottom: 20px;
    }
    .main__profile-img{
        width: 40px;
        height: 40px;
        border-radius: 50%;
        border: 0;
        vertical-align: middle;
        position: relative;
    }
    .main__profile-Setting-wrapper{
        position: absolute;
        bottom: 65px;
        left: 20px;
        padding: 20px 0;
        background-color: var(--color1);
        color: var(--color2);
        box-shadow: 0 0 4px var(--color4);
        border-radius: 5px;
        cursor: pointer;
    }
    .main__setting-list{
        list-style: none;
        padding:0;
    }
    .main__setting-item{
         padding: 6px 30px;
        font-size: 15px;
         &:hover{
            background-color: var(--color3);
            color: var(--color1);
         }
    }
    .main__setting-item-links{
        text-decoration: none;
        color: var(--color2);
    }
    .user-list__addgroup-overlay{
        position: absolute;
        top: 0;
        right: 0; 
        bottom: 0;
        left: 0;
        background-color: rgba(0,0,0,.6);
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: center;
        z-index: 1;
    }
    .active{
        background-color: var(--border-color);
        color: var(--border-color);
        border-radius: 5px;
    }
</style>

